...
def post_project():
    ...
    employer_id = int(request.form.get("employer_id"))
...
def view_bids(project_id):
    ...
    if not bid:
        return jsonify({"message": "No bids yet"}), 200
...
def get_assignments(employer_id):
    ...
    return jsonify(data)
# MAKE PAYMENT
@employer_bp.route('/payment-summary/<int:assignment_id>', methods=['GET'])
def payment_summary(assignment_id):
    conn, cursor = get_cursor()
    ...
    """, (assignment_id,))
    agreed = cursor.fetchone()['agreed_amount']
...
@employer_bp.route('/payment', methods=['POST', 'OPTIONS'])
def make_payment():
    if request.method == 'OPTIONS':
        return '', 200  # ✅ CORS preflight fix
    ...
    if not assignment_id or not amount:
        return jsonify({"error": "Missing data"}), 400

    # 1️⃣ Get agreed amount
    cursor.execute("""
        SELECT agreed_amount FROM assignments WHERE id=%s
    """, (assignment_id,))
    row = cursor.fetchone()
    ...
    agreed_amount = float(row['agreed_amount'])
    ...
    if total_paid + float(amount) > agreed_amount:
        return jsonify({"error": "Payment exceeds agreed amount"}), 400
